...
import { logoutUtils } from '../utils/api';
...
const Chat = ({isAuth, setIsAuth}) => {
  ...
  const  clearNotifications = null;
  const handleNotificationClose = null;
  ...
  useEffect(() => {
     /// checking ifh the user accessing this page has a token
  ...
  useEffect(() => {
    ...
    //   setMessages(history);
    // });
    // listing for message emmited from d backend
    ...
    socket.on('connect_error',(err) =>{
      ...
      if(err.message.includes('Authentications error')){
        alert('Authentication failed. Please log in again');
        // we can logout , redirect / clear token etc
      }else{
        //other connection errrors(e.g., server down)
